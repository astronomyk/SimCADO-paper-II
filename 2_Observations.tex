...
Given that the dispersion time for stellar clusters is on the order of hundreds of millions of years \citep{Lada2003-ip}, at any point in time relatively few of the observable new clusters will be found in the ideal age range (between 5 and 20\,Myr) for studying the IMF. 
...
OB associations also provide a laboratory for studying the IMF with the obvious advantage that stellar density does not pose a problem. However, as OB associations are spread out over a few hundred parsecs, the chances of contamination from background sources is higher. 
