...
# from agpy.imf import make_cluster
...
clr = ["#B2EEEC", "#DAE0E0", "#E9ECB8", "#E4EE40", "#EECB40", "#EDB279",
       "#ED8C79", "#C3610F", "#C3610F"]
#          O          B        A         F         G         K         M        L          T

# Distance of well known objects in kpc
obj = ["130pc (Pleiades)", "450pc (Orion)", "8.5kpc (Galactic Centre)",
       "50kpc (LMC)",
       "770kpc (M31)", "2Mpc (NGC300)", "4Mpc (Cen A)", "5Mpc (M83)",
       "20Mpc (Virgo)"]
...
num = np.array([np.sum((masses < mass[i]) * (masses > mass[i + 1])) for i in
                range(len(mass) - 1)])
dm = 28 - np.array(Mk)
...
text_dist = mu_to_d(0.5 * (dm[:-1] + dm[1:])) / 1E3
for i in range(len(types)): plt.text(text_dist[i], 4E-3, types[i],
...
for i in range(len(mass)): plt.text(d_stars[i], 0.95E-3,
                                    str(mass[i]) + "M$_\odot$",
...
plt.ylabel("Stellar fraction", fontsize=18)
plt.tick_params(axis='both', which='major', labelsize=18)
plt.gca().xaxis.set_ticks_position('top')
plt.gca().xaxis.set_label_position('top')

for i in np.arange(0, 6): plt.text(10 ** i, 2, str(5 * (i + 2)) + " mag",
                                   horizontalalignment="center", fontsize=18)
plt.text(300, 3, "Distance Modulus [K band]", horizontalalignment="center",
         fontsize=18)

# Well known objects - vertical dashed line and dot point
text_dist = np.array([0.8] * (len(obj) - 5) + [1.12, 0.80, 0.80, 1.12, 0.80])
